...
- plots the abundance predictions of PriMiDM against those from Boehm 2014
- outputs pdfs for both EE and NU scenarios in BoehmEE.pdf and BoehmNU.pdf
- uses data from DHMcCabeEE.csv, DHMcCabeNU.csv, YMcCabeEE.csv, YMcCabeNU.csv, PRIMI_McCabe_EE_Maj, PRIMI_McCabe_NU_Maj
"""
...
if __name__ == '__main__':
	...
	for scenario in scenarios:
		primi = np.loadtxt('PRIMI_McCabe_{}_Maj'.format(scenario), skiprows=1)
		mass_arr = primi[:, 0]
		Yp_arr = 4*primi[:, 8]
		DoverH_arr = 10**5*primi[:, 5]/primi[:, 4]

		YpMcCabe = np.loadtxt('YMcCabe{}.csv'.format(scenario))
		DMcCabe = np.loadtxt('DHMcCabe{}.csv'.format(scenario))
		...
		for idx, color in enumerate(colors):
			...
			# 	lw=0.6,
			# 	label='')
			interpfn = interp1d(mass_arr, arrays[idx], kind='cubic')
			...
			if idx == 1:
				if scenario == 'EE':
					axes[idx].legend(markerfirst=markerfirstarr[idx], handlelength=3, loc=locs[idx], fontsize=14)
		...
		plt.savefig('Boehm{}.pdf'.format(scenario))	
