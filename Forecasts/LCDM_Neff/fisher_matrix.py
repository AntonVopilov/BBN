...
LCDM_params = {
    r"100\theta_\mathrm{s}":1.04090,
    r"\Omega_\mathrm{c}h^2":0.1202,
    r"\Omega_\mathrm{b}h^2":0.02236,
...
dh = {
    r"100\theta_\mathrm{s}": [2e-4],
    r"\Omega_\mathrm{c}h^2": [6e-4],
    r"\Omega_\mathrm{b}h^2": [3e-5],
    r"N_\mathrm{eff}": [0.03],
    r"\ln(10^{10}As)": [0.0095],
    r"n_\mathrm{s}": [0.002],
    r"\tau": [0.006]
}
...
for name, value in LCDM_params.items():
    params[name] = value + np.concatenate(([-dh[name][0]], [0.], dh[name]))
...
for name in params_names:
    ...
    for sp in spectra[name][spectra_names[0]]:
        sp[2970:] = 0.
    for sp2 in spectra[name][spectra_names[2]]:
        sp2[2970:] = 0.
    forimport.close()
...
planck_pos_relevant = [0, 1, 2, 3, 4, 6, 7]
fisher_planck_relevant = np.array([[fisher_planck[key1, key2] for key1 in planck_pos_relevant] for key2 in planck_pos_relevant])

planck_pos = [2, 1, 0, 6, 3, 4, 5]
planck_pos_sort = np.argsort(planck_pos)
fisher_planck_ordered = [[fisher_planck_relevant[key1, key2] for key1 in planck_pos_sort] for key2 in planck_pos_sort]

fisher_planck_stage4 = fisher_planck_ordered + fisher_matrix
covariance_planck_stage4 = np.linalg.inv(fisher_planck_stage4) # Planck 2018 + Stage-IV (Omegabh^2)
